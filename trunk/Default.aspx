...
<div class="h"><a href="p.aspx?s=<%=_storyID %><%=Helper.AppendUserName("&") %>">new</a> <a href="<%=Helper.AppendUserName("?")%>">refresh</a> <% if (Helper.AppendUserName("").Length > 0) { %> <a href="sm.aspx<%=Helper.AppendUserName("?") %>">marks</a> <%} %> <a href="sr.aspx<%=Helper.AppendUserName("?") %>">search</a> <% if (_threadUpdateCount > 0){ %> (<%= _threadUpdateCount%>) <%}%> <% if (Helper.AppendUserName("").Length > 0){ %><a href="s.aspx<%=Helper.AppendUserName("?")%>"><img src="set.gif" /></a><%}%></div>
...
<div class="c"><%#Helper.RelinkShackURLs(((System.Xml.XmlNode)Container.DataItem).FirstChild.InnerText.ToString().Trim(),"t.aspx")%><br /><br />
...
<asp:Literal runat="server" id="LiteralPages"></asp:Literal><br /><br />
<div class="h"><a href="p.aspx?s=<%=_storyID %><%=Helper.AppendUserName("&") %>">new</a> <a href="<%=Helper.AppendUserName("?")%>">refresh</a> <% if (Helper.AppendUserName("").Length > 0) { %> <a href="sm.aspx<%=Helper.AppendUserName("?") %>">marks</a> <%} %> <a href="sr.aspx<%=Helper.AppendUserName("?") %>">search</a> <% if (_threadUpdateCount > 0){ %> (<%= _threadUpdateCount%>) <%}%> <% if (Helper.AppendUserName("").Length > 0){ %><a href="s.aspx<%=Helper.AppendUserName("?")%>"><img src="set.gif" /></a><%}%></div>

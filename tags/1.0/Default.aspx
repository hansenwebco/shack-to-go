...
<div class="h"><a href="p.aspx?s=<%=_storyID %><%=Helper.AppendUserName("&") %>">new post</a> <a href="<%=Helper.AppendUserName("?")%>">refresh</a> <% if (Helper.AppendUserName("").Length > 0) { %> <a href="m.aspx<%=Helper.AppendUserName("?") %>">marks</a> <%} %> <% if (_threadUpdateCount > 0){ %> (<%= _threadUpdateCount%>) <%}%> <% if (Helper.AppendUserName("").Length > 0){ %><a href="s.aspx<%=Helper.AppendUserName("?")%>"><img src="set.gif" /></a><%}%></div>
